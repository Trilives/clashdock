...
| 命令 | 说明 |
|---|---|
| `clashdock` | **交互式主菜单（推荐）**：运行时管理 / 配置变更 / 工具 / 暂停启动 / 语言 / 卸载。服务尚未注册时会先选择语言，再询问是否现在进行初始化 |
| `clashdock init` | 初始化（首次部署）：下载代理→TUN→局域网→添加首个订阅（clash / base64 / 本地 YAML 文件三选一）→注册服务→可选更新内核/geo/UI。主服务启动前可 ^R 整体回退；启动后，资源更新与伴生单元安装按模块独立回退。交互式主菜单检测到服务未注册时会先选择语言再询问是否触发初始化；此子命令主要供脚本/无人值守场景显式调用 |
