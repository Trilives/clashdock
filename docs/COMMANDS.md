...
| `clashdock init` | 初始化（首次部署）：下载代理→TUN→局域网→订阅或导入本地 `config.yaml`→注册服务→可选更新内核/geo/UI，全程可 ^R 回退 |
| `clashdock modify` | 更改配置会话：订阅管理（含导入本地 `config.yaml`）/ 定制层 / 节点切换 / 更新 / 服务设置…（esc 保存，^R 回退） |
