...
| `clashdock init` | 初始化（首次部署）：先选语言→下载代理→TUN→局域网→订阅或导入本地 `config.yaml`→注册服务→可选更新内核/geo/UI，全程可 ^R 回退。交互式主菜单检测到服务未注册时会询问是否触发，此子命令主要供脚本/无人值守场景显式调用 |
| `clashdock modify` | 配置变更会话（对应主菜单「配置变更」）：订阅管理（含导入本地 `config.yaml`）/ 部署设置 / 自定义分流叠加（esc 保存，^R 回退）。节点切换/内核更新/自更新/服务设置等即时生效操作仅在交互式主菜单「运行时管理」里，无对应子命令 |
