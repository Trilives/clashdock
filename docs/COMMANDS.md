...
| `clashdock run` | **便携/轻量模式**（也用于从便携包目录自动进入）：不安装 systemd、不改系统路径、不需 root。选语言 → 在当前目录建 `./clashdock-data` 工作目录 → 添加订阅 → 用便携包自带内核前台直接启动（纯代理，本机 `127.0.0.1:7890`）。clashdock 停在前台监护内核，退出即停（菜单可切节点 / 看日志 / 重启 / 停止退出）。需完整服务改用便携包内 `install.sh` |
