...
| 命令 | 说明 |
|---|---|
| `clashdock` | **交互式主菜单（推荐）**：运行时管理 / 配置变更 / 工具 / 暂停启动 / 语言 / 卸载。启动第一步：配置文件未设置过语言（且未用 `CLASHDOCK_LANG` 指定）时先选语言；随后服务尚未注册时询问是否现在初始化。从解压的便携包目录启动会自动进入便携模式（见 `run`） |
| `clashdock run` | **便携/轻量模式**（也用于从便携包目录自动进入）：不安装 systemd、不改系统路径、不需 root。选语言 → 在可执行文件所在目录旁建 `clashdock-data` 工作目录（只读位置回退当前目录）→ 添加订阅 → 用便携包自带内核前台直接启动（纯代理，本机 `127.0.0.1:7890`）。clashdock 停在前台监护内核，退出即停（菜单可切节点 / 看日志 / 重启 / 停止退出）。需完整服务改用便携包内 `install.sh` |
