...
| 命令 | 说明 |
|---|---|
| `clashdock` | **交互式主菜单（推荐）**：暂停启动 / 运行时管理 / 配置变更 / 网络测试 / 语言 / 卸载。服务尚未注册（含已停止但单元仍在的情况）时自动先跑一遍初始化，无需单独选择 |
| `clashdock init` | 初始化（首次部署）：下载代理→TUN→局域网→订阅或导入本地 `config.yaml`→注册服务→可选更新内核/geo/UI，全程可 ^R 回退。交互式主菜单已改为自动触发，此子命令主要供脚本/无人值守场景显式调用 |
| `clashdock modify` | 配置变更会话（对应主菜单「配置变更（需重启生效）」）：订阅管理（含导入本地 `config.yaml`）/ 定制层（esc 保存，^R 回退）。节点切换/内核更新/服务设置等即时生效操作仅在交互式主菜单「运行时管理」里，无对应子命令 |
