...
- **工作目录**：当前目录下的 `./clashdock-data`（稳定命名，复用上次订阅），经
  `CLASHDOCK_HOME` 指过去；已设 `CLASHDOCK_HOME` 时尊重用户值。
...
- **监护**（`portable.Supervisor`）：`mihomo -d <runtime> -f config.yaml` 作为独立
  ...
  监护循环是一组阻塞式 `tui.Select`（切换节点走 `clashapi` 实时切、看日志、重启、
  停止并退出），复用既有交互组件，不引入自定义 Bubble Tea 模型。
