...
const usage = "用法: clashdock [init|modify|nettest|uninstall|update|pause|resume|version]\n不带参数则进入交互式主菜单。"
...
func interactive(p paths.Paths) int {
	idx := 0
	for {
		options := []string{"初始化（首次部署）", "更改配置", switchLabel(), "网络测试", "卸载所有服务"}
		i, err := tui.Select("mihomo 部署系统", options, tui.SelectOpts{BackLabel: "退出", Initial: idx})
		if err != nil {
			fmt.Println("再见。")
			return 0
		}
...
func serveUI(args []string) int {
	...
	port := fs.Int("port", sysd.DefaultWebUIPort, "监听端口")
	bind := fs.String("bind", "127.0.0.1", "绑定地址")
	dir := fs.String("dir", "", "静态文件目录")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if *dir == "" {
		execx.Error("webui-serve 需要 --dir")
		return 2
	}
	addr := fmt.Sprintf("%s:%d", *bind, *port)
	execx.Info(fmt.Sprintf("静态面板服务: http://%s/ ← %s", addr, *dir))
