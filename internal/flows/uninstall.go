...
func Uninstall(p paths.Paths) error {
	...
	chosen, err := tui.MultiSelect("卸载（勾选要移除的项）", items, []int{0, 1, 2, 3})
	if err != nil {
		return nil // 取消
	}
	if len(chosen) == 0 {
		execx.Info("未选择任何项，已取消。")
		return nil
	}
	execx.Header("即将卸载")
	...
	actions := []func() error{
		...
		func() error {
			...
			execx.Ok("已清理本地产物（内核 / UI / 缓存 / geo 数据）。")
			return nil
		},
		func() error {
			proxyenv.Remove() // 清掉 bashrc 代理变量，避免残留指向失效代理
			os.RemoveAll(p.State)
			execx.Ok("已清理状态目录（所有订阅与配置）。")
			return nil
		},
	}
	for _, i := range chosen {
		if err := actions[i](); err != nil {
			execx.Error(fmt.Sprintf("移除「%s」失败：%v", items[i], err))
		}
	}
	execx.Ok("卸载流程结束。")
	return nil
