...
// RunHealthcheck is the watchdog probe used by mihomo-watchdog.service.
// It restarts mihomo only when the host has a real uplink but the local proxy
// is unresponsive for several attempts.
func RunHealthcheck(args []string) int {
	fs := flag.NewFlagSet("healthcheck", flag.ContinueOnError)
	service := fs.String("service", envOr("SERVICE_NAME", DefaultName), "systemd service name")
	tunDev := fs.String("tun-dev", envOr("TUN_DEV", "mihomo"), "TUN interface to ignore")
...
func runtimeProxyAddr(service string) string {
	...
	if port := jsonNumber(cfg["mixed-port"]); port > 0 {
		return "127.0.0.1:" + strconv.Itoa(port)
	}
	if port := jsonNumber(cfg["port"]); port > 0 {
		return "127.0.0.1:" + strconv.Itoa(port)
	}
	return ""
}
