// 每周自动更新定时器：周期性更新 内核/UI/geo 数据并同步重启服务。
// ExecStart 调用 clashdock 自身的 `update` 子命令。
package sysd
...
Description=Weekly mihomo core, UI, and rule-set update (%s)
