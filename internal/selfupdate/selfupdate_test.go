...
func TestExtractTarGz(t *testing.T) {
	dir := t.TempDir()
	archive := writeTarGz(t, dir, map[string]string{"clashdock": "binary-content", "README.md": "hi"})
	...
	got, err := os.ReadFile(filepath.Join(outDir, "clashdock"))
	if err != nil {
		t.Fatalf("clashdock not extracted: %v", err)
	}
...
func TestVerifySHA256(t *testing.T) {
	dir := t.TempDir()
	archivePath := filepath.Join(dir, "clashdock_1.0.0_linux_amd64.tar.gz")
	if err := os.WriteFile(archivePath, []byte("payload"), 0o644); err != nil {
		t.Fatal(err)
	}
	sum := sha256.Sum256([]byte("payload"))
	sumsPath := filepath.Join(dir, "checksums.txt")
	content := hex.EncodeToString(sum[:]) + "  clashdock_1.0.0_linux_amd64.tar.gz\n"
	if err := os.WriteFile(sumsPath, []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := verifySHA256(archivePath, sumsPath, "clashdock_1.0.0_linux_amd64.tar.gz"); err != nil {
...
func TestAssetName(t *testing.T) {
	...
	if !bytes.Contains([]byte(name), []byte("clashdock_0.1.2_linux_")) {
