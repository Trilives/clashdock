...
// 默认路径经云端 subconverter（target=clash）转换；应急路径（base64_local_fallback，
// 默认关闭）本地直接解析节点分享链接为 Clash 风格 proxy 字典（best-effort，
// 覆盖 vmess/ss/trojan/vless/hysteria2/tuic，格式怪异时可能漏字段/转错）。
...
func ToClashDict(rawText string, cfg map[string]any) (map[string]any, error) {
	backend := strOr(cfg["subconverter_backend"], "")
	proxy := strOr(cfg["download_proxy"], "")
	localFallback := truthy(cfg, "base64_local_fallback", false)
