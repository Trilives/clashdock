...
（订阅管理 + 定制层编辑，写的是 mihomo 运行配置本身）与 `flows.ModifyRuntime`（节点切换 /
内核更新 / 服务设置 / Web 面板 / 网络自愈 / 更新定时器，均即时生效），两者共享同一套
`modifySession` 快照 + 回退骨架。交互式主菜单不再有单独的「初始化」入口：进入主菜单前
若 `sysd.IsInstalled` 判定主服务未注册（已停止但单元文件仍在也算已注册），自动跑一遍
`flows.Init`。各级菜单选项按常用程度排列（日常操作在前，卸载类低频/破坏性操作在后）。
...
internal/flows        流程编排（init / modifyconfig / modifyruntime / nettest / uninstall / nodeselect / 定制层编辑）
```
