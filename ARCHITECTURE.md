...
内核/geo 数据下载不再询问是否下载、是否走代理——自动尝试（服务启动前保持既有
直连优先/`download_proxy`兜底；服务启动后优先走本机 mixed-port，见第 5 节）；
下载失败也**不回滚**已完成的 TUN/代理/局域网设置与已添加的订阅，只警告并提示
去「运行时管理 → 更新」补下载，重新执行初始化即可（会识别已有订阅，跳过重复
添加，直接重试服务注册）。
...
判定，已停止但单元文件仍在也算已注册）时会先触发语言选择（`flows.PickLanguage`），
再询问是否现在进行初始化，而不是自动强制进入。各级菜单选项按常用程度排列（日常操作在前，
